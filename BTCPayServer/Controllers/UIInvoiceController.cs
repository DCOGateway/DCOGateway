            ...
            if (entity.Type == InvoiceType.TopUp || entity.Price != 0m)
            {
                ...
                if (contexts.Count == 0)
                {
                    ...
                    if (!store.GetPaymentMethodConfigs(_handlers).Any())
                        message.AppendLine(
                            "No wallet has been linked to your BTCPay Store. See the following link for more information on how to connect your store and wallet. (https://docs.dcogate.org/WalletSetup/)");
                    else
                    {
