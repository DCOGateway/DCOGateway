...
namespace BTCPayServer.Controllers
{
    ...
    public partial class UIServerController : Controller
    {
        ...
        public async Task<IActionResult> DeleteDynamicDnsService(string hostname)
        {
            ...
            return View("Confirm",
                new ConfirmModel("Delete dynamic DNS service",
                    $"Deleting the dynamic DNS service for <strong>{Html.Encode(hostname)}</strong> means your DCO Gateway will stop updating the associated DNS record periodically.", StringLocalizer["Delete"]));
        }
        ...
        {
            return View("Confirm", new ConfirmModel(StringLocalizer["Disable modification of SSH settings"], StringLocalizer["This action is permanent and will remove the ability to change the SSH settings via the DCO Gateway user interface."], StringLocalizer["Disable"]));
        }
