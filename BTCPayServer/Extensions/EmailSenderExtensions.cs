...
namespace BTCPayServer.Services
{
    public static class EmailSenderExtensions
    {
        ...
        private static string HEADER_HTML = "<h1 style='font-size:1.2rem'>DCO Gateway</h1><br/>";
        ...
        public static void SendResetPassword(this IEmailSender emailSender, MailboxAddress address, string link)
        {
            emailSender.SendEmail(address, "Update Password", CreateEmailBody(
                $"A request has been made to reset your DCO Gateway password. Please set your password by clicking below.<br/><br/>{CallToAction("Update Password", link)}"));
        }
        ...
        public static void SendInvitation(this IEmailSender emailSender, MailboxAddress address, string link)
        {
            emailSender.SendEmail(address, "Invitation", CreateEmailBody(
                $"<p>Please complete your account setup by clicking <a href='{HtmlEncoder.Default.Encode(link)}'>this link</a>.</p><p>You can also use the DCO Gateway app and scan this QR code when connecting:</p>{GetQrCodeImg(link)}"));
        }
